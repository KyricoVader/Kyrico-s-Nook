    ...
    return(
        <div className="navbar">
            ...
            <div className='navbar-search'>
                ...
                <img src={search} alt="Article" className="navbar-search-icon" />
            </div>
            <div className="navbar-link-container">
                {navbarIcon.map((icon, index) => (
                    <Link to="/home-page" className="navbar-link" key={index}>
                        <img src={icon} alt={`Article ${index + 1}`} className="navbar-icon" />
                    </Link>
                ))}
            </div>
        </div>
